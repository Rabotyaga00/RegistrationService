...
class SellerService(
    ...
    fun registerSeller(id: UUID, name: String): Seller {
        val existing = sellerRepository.findBySellerId(id)
        val existingUser = userClient.getUserById(id)
        if (existingUser != null) throw IllegalArgumentException("User $id already exists")
        if (existing != null) throw IllegalArgumentException("Seller already exists")
